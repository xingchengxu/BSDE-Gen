...
def model_inference(input_noise, img_output=False, model_store_path=None):
    ...
    if img_output == True:
        ...
        plt.show()
        # plt.savefig("fbsde_gen_mnist.png")
        plt.close()
...
for row in range(n_row):
    for col in range(n_col):
        ...
        # # Clean method I:
        # y_output = torch.sigmoid(y_output).to(device)
        # q_u = torch.quantile(y_output, 0.5, interpolation='midpoint')
        # q_l = torch.quantile(y_output, 0.2, interpolation='midpoint')
        # # y_trans[row * n_col + col, :] = torch.tensor([min(x,1) if x > q_u else
        # #                                               (x*0.8 if x>q_l else x/10)
        # #                                               for x in y_output.t()])
        # y_trans[row * n_col + col, :] = torch.tensor([x if x > q_l else 0 for x in y_output.t()])

        # # Clean method II:
        # y_temp = y_output.clone()
        # y_temp -= torch.min(y_temp)
        # y_temp *= 255 / torch.max(y_temp)
        # q_l = torch.quantile(y_temp, 0.33, interpolation='midpoint')
        # y_trans[row * n_col + col, :] = torch.tensor([x if x > q_l else torch.div(x, 10, rounding_mode='trunc').long() for x in y_temp.t()])
...
for row in range(n_row):
    for col in range(n_col):
        ...
        ax.get_yaxis().set_visible(False)
plt.show()
plt.savefig(image_store_path_r)
plt.close()
